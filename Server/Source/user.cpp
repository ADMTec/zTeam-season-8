...
// codex/audit-code-for-potential-issues
	...
	if ( lpObj->Experience >= 2147483647 && lpObj->Level < 400)
	{
		lpObj->Experience = 2147483648;
	}
	/*
	if ( lpObj->Leadership > 32767 )
	{
		lpObj->Leadership = 32767;
	}*/
	lpMsg->MapX = lpObj->X;
	lpMsg->MapY = lpObj->Y;
	lpObj->TX = lpMsg->MapX;
	...
	if ( (lpMsg->CtlCode & 8) == 8 )
	{
		...
		sprintf (Text, "[ADMIN] %s is now ONLINE!", lpObj-> Name);
	...
	{
		gObjAddBuffEffect(lpObj,BUFF_GM,0,0,0,0,-10);
		// Here for GM Online Announcement Fix
		/*
		char szTemp[128];
		...
		*/
		...
		char Text [256];
		ZeroMemory (Text, sizeof (Text));
		sprintf (Text, "[GM] %s is now ONLINE!", lpObj-> Name);
		AllSendServerMsg (Text);
	}
	...
	if ( bAllItemExist == false )
	{
		...
		TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 1, "MyServerName");	// Deathway need Translation
...
void gObjTradeOkButton(int aIndex)
{
	...
	//Harmony items + Elemental items can be traded
	/*
...
void gObjViewportListCreate(short aIndex)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		...
		{
			//Here bug crash -> 0xC0000005 -> The thread tried to read from or write to a virtual address for which it does not have the appropriate access. 
			...
			//	}
				...
				if(State != 0)
				{
					...
					if(lpObj->Type == OBJ_USER)
					{
						...
						//New Added to fix Arena Die
						/*
						else if(lpObj->MapNumber == MAP_INDEX_BATTLESOCCER)
						{
							lpObj->MapNumber == MAP_INDEX_RORENCIA;
							MapC[lpObj->MapNumber].GetMapPos(lpObj->X,lpObj->Y);
						}
						*/
						//else if(lpObj->MapNumber == 6 || lpObj->X > 56 || lpObj->Y > 90 )
						/*
						else if ( lpObj->MapNumber == 6 )
						{
							//lpObj->X = 72;
							//lpObj->Y = 140;
							lpObj->X = 62;
	...
	else
	{
		...
		if(percent != 0)
		{
			...
			if(lpObj->m_LifeFillCount > 6)
			{
				...
				// Added try fix for time for HP pot
				if(GetTickCount() - lpObj->Life > 10000)
				{
				lpObj->Life = tlife;
				}
...
void gObjTeleportMagicUse(int aIndex, BYTE x, BYTE y)	// Check if x and y are BYTE
{
	...
	{
		//LogAdd("[Anti-Hack][WrongPosition] Wrong Distance Magic [%s][%s]", lpObj->AccountID, lpObj->Name);
		LogAddTD("[Anti-Hack][WrongPosition] Wrong Distance Magic");
		CloseClient(aIndex);
		return;
	}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	//Here need to add proper fix for PK Die
...
CMagicInf * gObjGetMagicSearch(LPOBJ lpObj, int skillnumber)
{
	...
	//Summoner Master Drain Life Fix
	/*
	if(skillnumber == 462 )
	{
...
BOOL gCheckSkillDistance(int aIndex, int aTargetIndex, int iSkillNum)
{
	...
	{
		LogAddTD("[Anti-Hack][SkilDistanceCheck] [%s][%s] Invalid Skill attacker(%d, %d), defender(%d, %d), skillnum:%d skilldistance:%d",
			lpObj->AccountID, lpObj->Name,
			lpObj->X, lpObj->Y, 
