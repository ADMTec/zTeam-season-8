...
void ReadEventInfo(MU_EVENT_TYPE eEventType)
{
	...
	switch(eEventType)
	{
		case 0: //All Events
			...
			g_bDoBlueEvent = GetPrivateProfileInt("GameServerInfo","BlueEvent", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
			...
			g_BlueEvent.Load(gDirPath.GetNewPath(FILE_BLUE_EVENT));
...
#endif
			//Potion Delay Time
			g_PotionDelay = GetPrivateProfileInt("GameServerInfo", "PotionDelay", 0, gDirPath.GetNewPath(FILE_SERVER_COMMON));
			g_bPostFloodProtect = GetPrivateProfileInt("GameServerInfo", "IsPostFloodProtect", 1, gDirPath.GetNewPath(FILE_SERVER_COMMON));
			g_bPostFloodProtectTime = GetPrivateProfileInt("GameServerInfo", "PostFloodProtectTime", 1, gDirPath.GetNewPath(FILE_SERVER_COMMON));
			...
			if(g_bVaultFloodProtectTime > 15000)
				{
					g_bVaultFloodProtectTime = 15000;
				}
			//Potion Delay Time
