...
CElementalSystem::CElementalSystem()
{
	...
	m_DamageTable[ELEMENT_FIRE][ELEMENT_WIND] = 510;
	m_DamageTable[ELEMENT_FIRE][ELEMENT_DARK] = 520;

	m_DamageTable[ELEMENT_WATER][ELEMENT_FIRE] = 520;
	m_DamageTable[ELEMENT_WATER][ELEMENT_WATER] = 500;
	m_DamageTable[ELEMENT_WATER][ELEMENT_EARTH] = 500;
	m_DamageTable[ELEMENT_WATER][ELEMENT_WIND] = 500;
	m_DamageTable[ELEMENT_WATER][ELEMENT_DARK] = 510;

	m_DamageTable[ELEMENT_EARTH][ELEMENT_FIRE] = 510;
	m_DamageTable[ELEMENT_EARTH][ELEMENT_WATER] = 520;
	m_DamageTable[ELEMENT_EARTH][ELEMENT_EARTH] = 500;
	m_DamageTable[ELEMENT_EARTH][ELEMENT_WIND] = 580;
	m_DamageTable[ELEMENT_EARTH][ELEMENT_DARK] = 590;

	m_DamageTable[ELEMENT_WIND][ELEMENT_FIRE] = 590;
	m_DamageTable[ELEMENT_WIND][ELEMENT_WATER] = 510;
	m_DamageTable[ELEMENT_WIND][ELEMENT_EARTH] = 520;
...
void CElementalSystem::LoadOptions()
{
	...
	m_iLorenciaK_DropLevel = GetPrivateProfileInt("GameServerInfo","LorenciaK_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iLorenciaK_DropRate = GetPrivateProfileInt("GameServerInfo","LorenciaK_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));


	m_iIronS_DropLevel = GetPrivateProfileInt("GameServerInfo","IronS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iIronS_DropRate = GetPrivateProfileInt("GameServerInfo","IronS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iHeroE_DropLevel = GetPrivateProfileInt("GameServerInfo","HeroE_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iHeroE_DropRate = GetPrivateProfileInt("GameServerInfo","HeroE_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iBraveHE_DropLevel = GetPrivateProfileInt("GameServerInfo","BraveHE_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iBraveHE_DropRate = GetPrivateProfileInt("GameServerInfo","BraveHE_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iGladiatorsD_DropLevel = GetPrivateProfileInt("GameServerInfo","GladiatorsD_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iGladiatorsD_DropRate = GetPrivateProfileInt("GameServerInfo","GladiatorsD_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iMGladiatorsD_DropLevel = GetPrivateProfileInt("GameServerInfo","MGladiatorsD_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iMGladiatorsD_DropRate = GetPrivateProfileInt("GameServerInfo","MGladiatorsD_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iKMadnessB_DropLevel = GetPrivateProfileInt("GameServerInfo","KMadnessB_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iKMadnessB_DropRate = GetPrivateProfileInt("GameServerInfo","KMadnessB_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iKMagicSS_DropLevel = GetPrivateProfileInt("GameServerInfo","KMagicSS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iKMagicSS_DropRate = GetPrivateProfileInt("GameServerInfo","KMagicSS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iEmpireGS_DropLevel = GetPrivateProfileInt("GameServerInfo","EmpireGS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iEmpireGS_DropRate = GetPrivateProfileInt("GameServerInfo","EmpireGS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iAncientIS_DropLevel = GetPrivateProfileInt("GameServerInfo","AncientIS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iAncientIS_DropRate = GetPrivateProfileInt("GameServerInfo","AncientIS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iArcaP_DropLevel = GetPrivateProfileInt("GameServerInfo","ArcaP_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iArcaP_DropRate = GetPrivateProfileInt("GameServerInfo","ArcaP_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iAntoniasS_DropLevel = GetPrivateProfileInt("GameServerInfo","AntoniasS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iAntoniasS_DropRate = GetPrivateProfileInt("GameServerInfo","AntoniasS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));

	m_iKundunsSS_DropLevel = GetPrivateProfileInt("GameServerInfo","KundunsSS_DropLevel",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
	m_iKundunsSS_DropRate = GetPrivateProfileInt("GameServerInfo","KundunsSS_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
...
BOOL CElementalSystem::Drop(LPOBJ lpObj,LPOBJ lpTargetObj)
{
	...
	if( lpObj->MapNumber == MAP_INDEX_ACHERON )
	{
		...
		//Here to add new pentagram drop rate
	...
	// New Pentagram Start Here

		if( lpObj->Level >= m_iLorenciaK_DropLevel && rand()%10000 < m_iLorenciaK_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,202), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}
		...
		if( lpObj->Level >= m_iIronS_DropLevel && rand()%10000 < m_iIronS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,203), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iHeroE_DropLevel && rand()%10000 < m_iHeroE_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,204), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iBraveHE_DropLevel && rand()%10000 < m_iBraveHE_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,205), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iGladiatorsD_DropLevel && rand()%10000 < m_iGladiatorsD_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,206), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iMGladiatorsD_DropLevel && rand()%10000 < m_iMGladiatorsD_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,207), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iKMadnessB_DropLevel && rand()%10000 < m_iKMadnessB_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,208), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iKMagicSS_DropLevel && rand()%10000 < m_iKMagicSS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,209), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iEmpireGS_DropLevel && rand()%10000 < m_iEmpireGS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,210), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iAncientIS_DropLevel && rand()%10000 < m_iAncientIS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,211), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iArcaP_DropLevel && rand()%10000 < m_iArcaP_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,212), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iAntoniasS_DropLevel && rand()%10000 < m_iAntoniasS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,213), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}

		if( lpObj->Level >= m_iKundunsSS_DropLevel && rand()%10000 < m_iKundunsSS_DropRate )
		{
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,214), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}
...
int CElementalSystem::GetTargetDefense(LPOBJ lpObj,LPOBJ lpTargetObj,CMagicInf* lpMagic,BYTE btAttackElement)
{
	...
	if( lpTargetObj->Type == OBJ_USER )
	{
		...
		case ELEMENT_WATER:
			...
			break;
		case ELEMENT_EARTH:
			IncreaseDefense += lpTargetObj->m_ElementalOptions->m_AddEarthPercDef;
			break;
		case ELEMENT_WIND:
			IncreaseDefense += lpTargetObj->m_ElementalOptions->m_AddWindPercDef;
			break;
		case ELEMENT_DARK:
			IncreaseDefense += lpTargetObj->m_ElementalOptions->m_AddDarkPercDef;
			break;
		}
...
int CElementalSystem::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTargetObj,int targetdefense,CMagicInf* lpMagic,BYTE btAttackElement,BYTE btDefenseElement)
{
	...
	if( lpObj->Type == OBJ_USER )
	{
		...
		case ELEMENT_WATER:
			...
			break;
		case ELEMENT_EARTH:
			IncreaseDamage += lpObj->m_ElementalOptions->m_AddEarthPercDmg;
			break;
		case ELEMENT_WIND:
			IncreaseDamage += lpObj->m_ElementalOptions->m_AddWindPercDmg;
			break;
		case ELEMENT_DARK:
			IncreaseDamage += lpObj->m_ElementalOptions->m_AddDarkPercDmg;
			break;
		}
...
BOOL CElementalSystem::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic)
{
	...
	if ( AttackDamage >= 1 )
	{
		if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
		{
			...
			//Map Arena = NoN-PVP
			else if ( lpObj->MapNumber == MAP_INDEX_BATTLESOCCER || lpTargetObj->MapNumber == MAP_INDEX_BATTLESOCCER )
			{
				selfdefense = 0;
			}
			//Map Arena = NoN-PVP
...
// Here need to fix to have 100 % rate
...
void CElementalSystem::ErrtelMix(LPOBJ lpObj)
{
	...
	g_ChaosBox.LogChaosItem(lpObj, "Ertel Mix");
	LogAddTD("[Elemental System] Ertel Mix Start");
...
void CElementalSystem::ErrtelRankUp(LPOBJ lpObj,BYTE btRank)
{
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++)
	{
		...
		//------------------------------------------//
		// Optional: Elemental Talisman Of Luck     //
		//------------------------------------------//
			else if ( lpObj->pChaosBox[n].m_Type == ITEMGET(14, 189) )
			{
				iPentagramLuckyCharm++;
		...
		//---------------------------------------//
		// Optional: Elemental Talisman Of Chaos //
		//---------------------------------------//
			else if ( lpObj->pChaosBox[n].m_Type == ITEMGET(14, 190) )
			{
				iPentagramChaosCharm++;
		...
		//--------------------------------------------------------------------------------------------------//
		// Requirements: Errtel, Spirit Powder, Bless, Soul, Ancient Item +7 and Talisman of Luck or Chaos  //
		//--------------------------------------------------------------------------------------------------//
