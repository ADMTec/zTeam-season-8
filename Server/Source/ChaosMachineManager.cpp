...
bool ChaosMachineManager::StartMix(LPOBJ lpUser, short RecipeIndex)
{
	...
	for ( unsigned int Recipe = 0; Recipe < this->Recipes.size(); Recipe++)

	{
		if (this->Recipes[Recipe]->RecipeIndex == RecipeIndex)
		{
			...
			for (unsigned int Combination = 0; Combination < this->Recipes[Recipe]->Combinations.size(); Combination++)

			{
				...
				for (int Pos = 0; Pos < CHAOS_BOX_SIZE; Pos++)
				{
					...
					for (unsigned int n = 0; n < this->Recipes[Recipe]->Combinations[Combination].Ingredients.size(); n++)
			...
			for (int Pos = 0; Pos < CHAOS_BOX_SIZE; Pos++)
			{
				...
				for (unsigned int n = 0; n < this->Recipes[Recipe]->Combinations[ActiveCombination].Talismans.size(); n++)
