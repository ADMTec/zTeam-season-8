...
BOOL wsShopServerCli::FDWRITE_MsgDataSend()
{
	...
	while (*sendbuflen>0)
	{
		...
		// Here added to test is this section is broken
		LogAdd("error-L3_New : nDx %d m_SendBufLen %d", nDx, this->m_SendBufLen );
		...
		// Here added to test is this section is broken
		LogAdd("error-L3_Newwwww : nDx %d m_SendBufLen %d", nDx, this->m_SendBufLen );
	}
	return TRUE;
}
