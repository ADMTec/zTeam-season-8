...
CLogToFile g_GMChatLog2("GMSystem", ".\\LOG\\GMLog", TRUE);
CLogToFile g_PrivateChatLog("Private", ".\\LOG\\Chat", TRUE);
CLogToFile g_AntiHackLog("AntiHack", ".\\LOG\\AntiHack", TRUE);
					...
					switch ( lpDef->subcode )
					{
						...
						case 0x01:
								...
								aRecv[7] == 0x00 )
							{
								...
								//LogAddTD("[Anti-Hack] Trying to create character with < 1 symbols. Account: %s / Character Name: %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
								g_AntiHackLog.Output("[Anti-Hack] Trying to create character with < 1 symbols. Account: %s / Character Name: %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	else
	{
		...
		switch ( protoNum )
		{
					...
					if (gObj[aIndex].ChatTickCount != 0) 
					{
						int Tick = GetTickCount() - gObj[aIndex].ChatTickCount;
							if (Tick < 900) //in ms
							{
								...
								//LogAdd("[Anti-Hack] Chat Flood Detected. Account:[%s] / Character Name: [%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
								g_AntiHackLog.Output("[Anti-Hack] Chat Flood Detected. Account:[%s] / Character Name: [%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);

								::GCServerMsgStringSend("Need to wait 1 second before you can type again!", aIndex, 1);
			...
			case 0x01:
				...
				//Normal Value
				//LogAddTD ("[Anti-Hack] Player used Spoof Hack : Account: %s / Character Name: %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
				g_AntiHackLog.Output("[Anti-Hack] Player used Spoof Hack : Account: %s / Character Name: %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
				GCServerMsgStringSend("We dont allow cheaters here! Next time you will get ban.",aIndex,0);
				CloseClient(aIndex);
				break;
			case 0x02:
				...
				//Normal Value
				    {
						if (aLen > 5 || aRecv[3] > 7)
						{
							g_AntiHackLog.Output("[Anti-Hack] Player used Disconnect Hack : Account: %s / Character Name: %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
							CloseClient(aIndex);
							//return;
						}
				break;
			...
			case 0x55:
				...
				CGGuildMasterInfoSave(aIndex, (PMSG_GUILDINFOSAVE *)aRecv);
				{
					if (gObj[aIndex].Connected != 3)
					 {
						 g_AntiHackLog.Output("[Anti-Hack][Create Guild] [Not in playing state] [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
						 CloseClient(aIndex);
						 //return;
					 }
					...
					for(int i = 0; i < Length; ++i)
						{
							if(isalnum(aRecv[4+i] == 0))
								{
									g_AntiHackLog.Output("[Anti-Hack][Create Guild] [Restricted letters] [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
									CloseClient(aIndex);
									// return;
								}
						}
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex)
{
	...
	if ( slen > MAX_CHAT_LEN-1 )
	{
		...
		g_AntiHackLog.Output("[Anti-Hack][PChatProc][%s][%s] Chat Message Len : %d", lpObj->AccountID, lpObj->Name, slen);
	...
	switch ( lpChat->chatmsg[0] )
	{
		...
		case '!': // Global Announcement + [GM] Name
			if ( slen > 2 )
			{
					if(CheckAuthority(34,lpObj) == 1)
						{
							...
							sprintf(Text,"[%s] %s", lpObj->Name,&lpChat->chatmsg[1]);
							AllSendServerMsg(Text);

							g_GMChatLog2.Output(lMsg.Get(MSGGET(1, 215)), gObj[aIndex].AccountID, gObj[aIndex].Name, &lpChat->chatmsg[1]);
							//LogAddTD(lMsg.Get(MSGGET(1, 215)), gObj[aIndex].AccountID, gObj[aIndex].Name, &lpChat->chatmsg[1]);
...
void CGChatWhisperRecv(PMSG_CHATDATA_WHISPER* lpMsg, int aIndex)
{
	...
	if ( len > 0 )
	{
		...
		//Private Chat Log System
		//Full Log
		//g_PrivateChatLog.Output("Account: [%s] Name: [%s] -> Account: [%s] Name: [%s] Message: [%s]", lpObj->AccountID, lpObj->Name, gObj[index].AccountID, gObj[index].Name, lpMsg->chatmsg);
		//Simple Log
		g_PrivateChatLog.Output("Name: [%s] -> Name: [%s] Message: [%s]", lpObj->Name, gObj[index].Name, lpMsg->chatmsg);
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		...
		//	gObj[aIndex].AccountID, gObj[aIndex].Name);
		g_AntiHackLog.Output("[Anti-Hack][CGPCharacterCreate] Attempted Character Create during GamePlay. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPCharDel(PMSG_CHARDELETE * lpMsg,int aIndex)
{
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		...
		//LogAddTD("[Anti-HACK][CGPCharDel] Attempted Character Delete during GamePlay. [%s][%s]",
			//gObj[aIndex].AccountID, gObj[aIndex].Name);
		g_AntiHackLog.Output("[Anti-Hack][CGPCharDel] Attempted Character Delete during GamePlay. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	if ( gObj[aIndex].bEnableDelCharacter == FALSE )
	{
		...
		//LogAddTD("[Anti-HACK][CGPCharDel] Attempted Character Delete during Prevented Time. [%s][%s]",
			//gObj[aIndex].AccountID, gObj[aIndex].Name);
		g_AntiHackLog.Output("[Anti-Hack][CGPCharDel] Attempted Character Delete during Prevented Time. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type)
{
	...
	if ( gObj[aIndex].m_bMapSvrMoveQuit == true )
	{
		...
		//LogAddTD("[ANTI-HACK][Item Duplication] - Item Drop during MapServer Move [%s][%s]",
			//gObj[aIndex].AccountID, gObj[aIndex].Name);
		g_AntiHackLog.Output("[Anti-Hack][Item Duplication] - Item Drop during MapServer Move [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	if ( pResult.Result == 1 )
	{
		...
		if ( bIsItem == TRUE )
		{
			...
			else if( g_SummonTheDemons.IsScroll(type) )
			{
				if( g_SummonTheDemons.DropScroll(lpObj, type) )
				{
					LogAddTD("[SummonTheDemons] [%s][%s] Success to create Golden",
						gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].pInventory[lpMsg->Ipos].m_Number);
					gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
				}
				else
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex)
{
	...
	if ( !gObjIsConnectedGP(aIndex) )
	{
		...
		//LogAddTD("[Anti-Hack][NPC Talk] Couldnt talk Account: [%s] Name: [%s]",
			//gObj[aIndex].AccountID, gObj[aIndex].Name);
		g_AntiHackLog.Output("[Anti-Hack][NPC Talk] Couldnt talk Account: [%s] Name: [%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	{
		//Here to add warning message to user
		::GCServerMsgStringSend("You cannot put items under level 6 in Store!", aIndex, 1);
		//Here to add warning message to user
	...
	if ( gObjCheckSerial0ItemList(sitem) != FALSE )
	{
		...
		//Here to add warning message to user
		::GCServerMsgStringSend("You cannot put items from NPCs in Store!", aIndex, 1);
		//Here to add warning message to user
		LogAddTD("[Anti-Hack][Serial 0 Item] [PShop] Account:[%s][Name:%s] Item:[%s] Pos:[%d]",
	...
	{
		//Here to add warning message to user
		::GCServerMsgStringSend("Item price must be at least 1!", aIndex, 1);
		//Here to add warning message to user
...
void CGPShopReqOpen(PMSG_REQ_PSHOP_OPEN * lpMsg, int aIndex)
{
	...
	{
		//Here to add warning message to user
		::GCServerMsgStringSend("Item level must be at least 6!", aIndex, 1);
		//Here to add warning message to user
		LogAddTD("[PShop] [%s][%s] ERROR : Level is Under 6 : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Level);
...
void CGPartyRequestRecv(PMSG_PARTYREQUEST * lpMsg, int aIndex)
{
	...
	{
		g_AntiHackLog.Output("[Anti-Hack] [PartyWithNPC] : Request Npc:[%d] Ip:[%s] Account:[%s] Name:[%s] State:[%d]", gObj[number].Class, gObj[aIndex].Ip_addr, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Connected);
		CloseClient(aIndex);
		return;
	}
	...
	//Arca war cant give party except your guild
	/*
#if (ENABLETEST_ARCA == 1)
...
void CGPartyRequestResultRecv(PMSG_PARTYREQUESTRESULT * lpMsg, int aIndex)
{
	...
	if( gObj[number].Type != OBJ_USER ) //season4 add-on
	{
		g_AntiHackLog.Output("[Anti-Hack] [TradeWithNPC]: Result: Npc:[%d] Ip:[%s] Account:[%s] Name:[%s] State:[%d]", gObj[number].Class, gObj[aIndex].Ip_addr, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Connected);
		CloseClient(aIndex);
		return;
	}
...
void CGGuildRequestRecv(PMSG_GUILDJOINQ * lpMsg, int aIndex)
{
	...
	if( gObj[number].Type != OBJ_USER ) //season4 add-on
	{
		g_AntiHackLog.Output("[Anti-Hack] [GuildWithNPC] : Request Npc:[%d] Ip:[%s] Account:[%s] Name:[%s] State:[%d]",  gObj[number].Class, gObj[aIndex].Ip_addr, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Connected);
		CloseClient(aIndex);
		return;
	}
...
void CGGuildRequestResultRecv(PMSG_GUILDQRESULT * lpMsg, int aIndex)
{
	...
	if( gObj[number].Type != OBJ_USER ) //season4 add-on
	{
		g_AntiHackLog.Output("[Anti-Hack] [GuildWithNPC] : Result Npc:[%d] Ip:[%s] Account:[%s] Name:[%s] State:[%d]", gObj[number].Class, gObj[aIndex].Ip_addr, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Connected);
		CloseClient(aIndex);
		return;
	}
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type)
{
	...
	if ( gObj[aIndex].m_IfState.use > 0 && gObj[aIndex].m_IfState.type != 12)
    {
            ...
            //LogAdd("[Anti-Hack][Trade and War] Account:[%s] Name:[%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
			g_AntiHackLog.Output("[Anti-Hack][Trade and War] Account:[%s] Name:[%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	if ( lpNode )
	{
		...
		if ( warmaster >= 1 )
		{
			...
			if ( gObj[aIndex].MapNumber == MAP_INDEX_ACHERON || gObj[warmaster].MapNumber == MAP_INDEX_ACHERON )
			{
				GCServerMsgStringSend("Guild War cannot be started within Acheron.", aIndex, 1);
				return;
			}

			if ( gObj[aIndex].MapNumber == MAP_INDEX_ACHERON_ARCABATTLE || gObj[warmaster].MapNumber == MAP_INDEX_ACHERON_ARCABATTLE )
			{
				GCServerMsgStringSend("Guild War cannot be started within Arca Battle.", aIndex, 1);
				return;
			}

			if ( gObj[aIndex].MapNumber == MAP_INDEX_DEBENTER || gObj[warmaster].MapNumber == MAP_INDEX_DEBENTER )
			{
				GCServerMsgStringSend("Guild War cannot be started within Debenter.", aIndex, 1);
				return;
			}

			if ( gObj[aIndex].MapNumber == MAP_INDEX_DEBENTER_ARCABATTLE || gObj[warmaster].MapNumber == MAP_INDEX_DEBENTER_ARCABATTLE )
			{
				GCServerMsgStringSend("Guild War cannot be started within Arca Battle2.", aIndex, 1);
				return;
			}
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex)
{
	...
	switch ( iMixType )	// TEST - Good
	{
		...
		case CHAOS_TYPE_SUMMONTHEDEMONS:
			g_SummonTheDemons.Mix(lpObj);
			break;
...
void RecvPositionSetProc(PMSG_POSISTION_SET * lpMove, int aIndex)
{
	...
	if ( ::gObjPositionCheck(lpObj) == FALSE )
	{
		...
		//LogAddTD("[Anti-Hack] Account: [%s] Name: [%s] try to move higher than 17 coords", lpObj->AccountID,lpObj->Name);
		g_AntiHackLog.Output("[Anti-Hack] Account: [%s] Name: [%s] try to move higher than 17 coords", lpObj->AccountID,lpObj->Name);
	...
	if ( !gCheckSkillDistance(aIndex, usernumber, lpMagic->m_Skill))
	{
		...
		if ( lpObj->m_iSkillDistanceErrorCount > g_iSkillDistanceKickCount )
		{
			if ( g_iSkillDistanceKick )
			{
				...
				//LogAddTD("[Anti-Hack][SKILL DISTANCE CHECK] [%s][%s] Kick Invalid Skill Area User. count(%d)",
					//lpObj->AccountID, lpObj->Name, lpObj->m_iSkillDistanceErrorCount);
				g_AntiHackLog.Output("[Anti-Hack][SKILL DISTANCE CHECK] [%s][%s] Kick Invalid Skill Area User. count(%d)", lpObj->AccountID, lpObj->Name, lpObj->m_iSkillDistanceErrorCount);
	...
	for (n=0;n<lpCount->Count;n++)
	{
		...
		for (i=0;i<lpCount->Count;i++)
		{
			if ( n != i)
			{
				...
				{
					LogAddTD("Account: [%s] Name: [%s] %s Detect Hack : Multi Attack",
						gObj[aIndex].AccountID, gObj[aIndex].Name,
	...
	if ( citem->IsItem() )
	{
		...
		else if( citem->m_Type == ITEMGET(14,162) )
		{
			...
			else
			{
				...
				//This Working
				//gObj[aIndex].pInventoryExtend >= 0;
...
void CGReqJewelMix(PMSG_REQ_JEWEL_MIX * lpMsg, int iIndex)
{
	...
	if( lpObj->m_IfState.use == 1 && lpObj->m_IfState.type == 1)
	{
		...
		//LogAddTD("[Anti-Hack][Lahap MixRequest] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
		g_AntiHackLog.Output("[Anti-Hack][Lahap MixRequest] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
...
void GCAnsJewelMix(int iIndex, int iResult)
{
	...
	if( lpObj->m_IfState.use == 1 && lpObj->m_IfState.type == 1)
	{
		...
		//LogAddTD("[Anti-Hack][Lahap Jewel Mix] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
		g_AntiHackLog.Output("[Anti-Hack][Lahap Jewel Mix] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
...
void CGReqJewelUnMix(PMSG_REQ_JEWEL_UNMIX * lpMsg, int iIndex)
{
	...
	if( lpObj->m_IfState.use == 1 && lpObj->m_IfState.type == 1)
	{
		...
		//LogAddTD("[Anti-Hack][Lahap JewelUnMix] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
		g_AntiHackLog.Output("[Anti-Hack][Lahap JewelUnMix] Account:[%s] Name:[%s]", lpObj->AccountID, lpObj->Name);
