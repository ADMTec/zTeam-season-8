...
void MapClass::SetRegentRect(int map)
{
	...
	case MAP_INDEX_BATTLESOCCER:
		...
		break;
		/*
	case MAP_INDEX_ARENA:
		SetRect(&this->gRegenRect, 72, 140, 73, 181);
		break;
		*/
...
void MapClass::GetMapPos(BYTE& ox, BYTE& oy)
{
	...
	while ( loopcount-- != 0 )
	{
		...
		__try
		{
			...
			//x = (BYTE)(this->gRegenRect.left + (rand()%w+1));
			//y = (BYTE)(this->gRegenRect.top  + (rand()%h+1));

			//Almost Fixed
			//x = (BYTE)(this->gRegenRect.left + Random(0,w-1));
			//y = (BYTE)(this->gRegenRect.top  + Random(0,h-1));
...
LPSTR MapAttribute::GetFile(int MapNumber, BYTE EventState)
{
	...
	for( size_t i = 0; i < this->m_Attribute.size(); i++ )
