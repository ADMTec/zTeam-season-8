...
int  CMoveCommand::GetMoveLevel(int mapnumber, int x, int y, int Class)
{
	...
	for( size_t i=0;i< this->m_MoveLevel.size();i++)
...
BOOL CMoveCommand::CheckEquipmentToMove(LPOBJ lpObj, int iTargetMapNumber)
{
	if ( iTargetMapNumber == MAP_INDEX_ATHLANSE )
	{
		if ( lpObj->pInventory[8].IsItem() != FALSE )
		{
			...
			{
				//Here to write warning message to user
				return FALSE;
			}
...
BOOL CMoveCommand::CheckInterfaceToMove(LPOBJ lpObj)
{
	...
	{
		::GCServerMsgStringSend("Tricks not work here!", lpObj->m_Index, 1);
		return FALSE;
	}
	...
	if ( lpObj->m_bPShopOpen == true )
	{
		...
		::GCServerMsgStringSend("Need to close Personal Shop before move!", lpObj->m_Index, 1);
	...
	{
		::GCServerMsgStringSend("Need to close Trade before move!", lpObj->m_Index, 1);
	...
	{
		::GCServerMsgStringSend("Need to close Vault before move!", lpObj->m_Index, 1);
	...
	{
		::GCServerMsgStringSend("Need to close NPC before move!", lpObj->m_Index, 1);
