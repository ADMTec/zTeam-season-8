...
{		
		...
		&(lpIoCtxt->Overlapped), NULL) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != ERROR_IO_PENDING)
		{
			////g_CS.Lock();
			GetLastErrorMsg();			
			LogAdd("Error-L15: WSASend(%d) failed with error %d %s ", __LINE__, WSAGetLastError(), gObj[aIndex].Ip_addr);
			////g_CS.UnLock();
			CloseClient( aIndex );			
			return FALSE;
		}
...
{ 	
	...
	//Should be gObj[index].m_socket != INVALID_SOCKET
