...
CLogToFile g_AntiHackLog3("AntiHack", ".\\LOG\\AntiHack", TRUE);
...
void GDGuildNoticeSave(LPSTR guild_name, LPSTR guild_notice)
{
	...
	// Fixing Guild Injection and Guild Crash Bug
	for(int i = 0; i < len;i++)
	{
		if(guild_notice[i] == '\'' || guild_notice[i] == '"' || guild_notice[i] == '%')
			return;
	}
	/*
	...
	*/
	...
	//Fix Guild Notice Client Crash
	if ( len > MAX_CHAT_LEN_OLD-2 )
	{
		return;
	}
	//Fix Guild Notice Client Crash
...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
	...
	{
		g_AntiHackLog3.Output("[Anti-Hack] FriendAddRequest to NPC: NPC:[%d] IP:[%s] Account:[%s] Name:[%s] State:[%d]", gObj[aTargetIndex].Class, gObj[aIndex].Ip_addr, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Connected);
		CloseClient(aIndex);
		return;
	}
