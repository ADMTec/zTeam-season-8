...
void CItem::Value()
{
		  ...
		  (m_Type <  ITEMGET(13,109) || m_Type > ITEMGET(13,115))) //New Rings / NeckLaces
	{
		...
		int lc16 = (int)m_SellMoney*0.6*lc15;
		m_SellMoney -= lc16;
	}
...
int CItem::DurabilityDown(int dur, int iIndex)
{
	...
	{
		DurCheck += (int)DurCheck*gObj[iIndex].m_MPSkillOpt.MpsDownDur2/100.0f;
	}
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			while ( true )
			{
				...
				strcpy_s(&p->Name[0],32, TokenString );
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			while ( true )
			{
				...
				strcpy_s(&p->Name[0],32, WzMemScript.GetString() );
